...
import.fn.data<-function(mydir, yr, program, mytables=all.tables) {
 all.tables<-c('FN121', 'FN123','FN124','FN125','FN126','FN127')
  if (length(yr)>1) {
 ...
 if(program == 'TW1') {all.fndata<-suppressWarnings(tw1.vartypes(year1))} # not sure what is causing the NAs introduced by coercion error here.
 
 if(program == 'GL1' & 'FN121' %in% mytables) {
  ...
  if(program == 'TW1'& 'FN121' %in% mytables) {
